...
class CoverageRunner {
	...
	public setupCoverage(): void {
		...
		this.transformer = (
			code: string,
			options: iLibHook.TransformerOptions,
		): string => {
	...
	public reportCoverage(): void {
		...
		this.matchFn.files.forEach((file) => {
			if (!cov[file]) {
				this.transformer(fs.readFileSync(file, "utf-8"), {
					filename: file,
				});
		...
		const reportTypes =
			this.options.reports instanceof Array
		...
		reportTypes.forEach((reportType) =>
			tree.visit(iReports.create(<any>reportType), context),
		);
	}
}
